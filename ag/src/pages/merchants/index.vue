    ...
    <div class="template-page">
        ...
        <div class="table_box">
            <a-table :columns="columns" :data-source="tableList" :pagination="pagination"
                ...
                <span slot="merchant_type" slot-scope="text">{{ text === 1 ? '直属商户' : '代理商户' }}</span>
                ...
                <span slot="action" slot-scope="text, record">
                    <a @click="recordUpdate(record)" v-if="record.id != merchant_id">编辑</a>
                </span>
            </a-table>
        </div>
            ...
            <a-form-model ref="ruleForm" :label-col="{ span: 6 }" :model="form" :rules="rules" :wrapper-col="{ span: 14 }">
                ...
                <a-form-model-item label="国家限制" prop="country_list">
                    <template v-for="country in countries">                    
                        <a-checkbox v-model="country_list[country.id]" :value="country.id" :checked="true" style="width: 100px;" > {{country.name }}</a-checkbox>
                        <a-input-number v-model="price_list[country.id]" placeholder="输入单价" :defaultValue="0.01" :value="0.01" :min="0.01" :max="0.99" :step="0.01" /><br />
                    </template>
                </a-form-model-item>
...
export default {
    ...
    data() {
        return {
            ...
            form: {
                ...
                merchant_name: "", merchant_type: 2, phone: "1234578", mail: "merchant@mail.com", remark: "请输入备注", country_list: "",
            },
            rules: {
    ...
    methods: {
        ...
        getTableData() {
            ...
            crudService.getRecords(`${BASE_URL}/auth/merchants`, this.search).then(res => {
                ...
                if (result.code === 0) {
                    ...
                    if (result.data.country_fees) { 
                        this.country_fees = result.data.country_fees;
                        for (let i = 0; i < this.country_fees.length; i++) { 
                            let item = this.country_fees[i];
        ...
        recordCreate() {
            ...
            this.form.mail = "mail" + num + "@gmail.com";
