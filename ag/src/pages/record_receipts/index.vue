    ...
    <div class="template-page">
        ...
        <div class="table_box">
            <a-table :columns="columns" :data-source="tableList" :pagination="pagination"
                ...
                <span slot="request_time" slot-scope="text">{{ datetime_ms(text) }}</span>
            </a-table>
        </div>
    </div>
</template>
...
export default {
    ...
    data() {
        return {
            ...
            columns: [
                ...
                {title: "订单编号", dataIndex: 'order_number', width: 165},
                {title: "商户名称", dataIndex: 'merchant_name', width: 120},
                {title: "手机号码", dataIndex: 'phone_full', width: 140},
                {title: "回执编码", dataIndex: 'message_id', width: 240},
                {title: "查询结果", dataIndex: 'request_result' },
                {title: "添加时间", dataIndex: 'created', scopedSlots: {customRender: 'created'}, width: 145},
                {title: "查询时间", dataIndex: 'request_time', scopedSlots: {customRender: 'request_time'}, width: 145},
            ],
            tableList: [],
            fileList: [],
    ...
    methods: {
        ...
        getTableData() {
            ...
            crudService.getRecords(`${BASE_URL}/auth/record_receipts`, this.search).then(res => {
                let result = res.data;
                if (result.code === 0) {
