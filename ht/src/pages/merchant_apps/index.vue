    ...
    <div class="template-page">
        <div class="search">
            <a-form-model ref="search" :model="search">
                <a-form-model-item label="所属商户" prop="state">
                    <a-select v-model="search.merchant_id" placeholder="请选择商户">
                        <a-select-option v-for="item in merchants" :key="item.id" :value="item.id">{{item.name}}</a-select-option>
                    </a-select>
                </a-form-model-item>
        ...
        <a-modal :centered="true" :confirm-loading="confirmLoading" :title="title" :visible="visible" @cancel="formCancel" @ok="formSubmit">
            <a-form-model ref="ruleForm" :label-col="{ span: 6 }" :model="form" :rules="rules" :wrapper-col="{ span: 16 }">
                <a-form-model-item label="所属商户" prop="state">
                    <a-select v-model="form.merchant_id" placeholder="请选择商户">
                        <a-select-option v-for="item in merchants" :key="item.id" :value="item.id">{{item.name}}</a-select-option>
                    </a-select>
                </a-form-model-item>
                <a-form-model-item label="应用名称" prop="name">
