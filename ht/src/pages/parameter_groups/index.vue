    ...
    <div class="template-page">
        <div class="search">
            <a-form-model ref="search" :model="search">
                <a-form-model-item label="分组名称" prop="title">
                    <a-input v-model="search.title" placeholder="请输入分组名称"/>
                </a-form-model-item>
                <a-form-model-item label="分组代码" prop="name">
                    <a-input v-model="search.name" placeholder="请输入分组代码"/>
                </a-form-model-item>
                <a-form-model-item label="分组备注" prop="remark">
            ...
            <div class="btns">
                <a-button type="primary" @click="rowCreate">添加参数分组</a-button>
            </div>
        </div>
        ...
        <a-modal :centered="true" :confirm-loading="confirmLoading" :title="title" :visible="visible" @cancel="formCancel" @ok="formSubmit">
            <a-form-model ref="ruleForm" :label-col="{ span: 6 }" :model="form" :rules="rules" :wrapper-col="{ span: 16 }">
                <a-form-model-item label="分组名称" prop="title">
                    <a-input v-model="form.title" placeholder="请输入分组名称"/>
                </a-form-model-item>
                <a-form-model-item label="分组名称" prop="name">
                    <a-input v-model="form.name" placeholder="请输入分组代码"/>
                </a-form-model-item>
                <a-form-model-item label="分组备注" prop="remark">
...
export default {
    ...
    data() {
        return {
            ...
            search: { page: 1, page_size: PAGE_SIZE, name: "", remark: "", title: "" },
            form: { name: "", remark: "", title: ""},
            rules: {
                title: [{required: true, message: "必须输入名称", trigger: "blur"}],
                name: [{required: true, message: "必须输入代码", trigger: "blur"}],
                remark: [{required: true, message: "必须输入备注", trigger: "blur"}],
            },
            columns: [
                {title: "ID", dataIndex: 'id', width: 80},
                {title: "分组名称", dataIndex: 'title', width: 200},
                {title: "分组代码", dataIndex: 'name', width: 200},
    ...
    methods: {
        getTableData() {
            ...
            crudService.getRecords(`${BASE_URL}/auth/parameter_groups`, this.search).then(res => {
                let result = res.data;
                if (result.code === 0) {
        ...
        formSubmit() {
            this.$refs.ruleForm.validate((valid) => {
                ...
                crudService.editRecord(`${BASE_URL}/auth/parameter_groups/save`, this.form).then((res) => {
                    let result = res.data;
                    if (result.code > 0) {
