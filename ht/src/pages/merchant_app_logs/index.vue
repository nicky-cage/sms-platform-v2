...
export default {
    name: "MerchantAppLogs",
    data() {
        return {
            is_creating: false,
            confirmLoading: false,
            search: {page: 1, page_size: PAGE_SIZE, merchant_name: "", app_name: "", remark: "" },
            columns: [
                {title: "ID", dataIndex: 'id', width: 80},
                {title: "商户名称", dataIndex: 'merchant_name', width: 150},
                {title: "应用名称", dataIndex: 'app_name', width: 150},
                {title: "调用IP", dataIndex: 'ip', width: 200},
    ...
    methods: {
        ...
        getTableData() {
            ...
            crudService.getRecords(`${BASE_URL}/auth/merchant_app_logs`, this.search).then(res => {
                let result = res.data;
                if (result.code === 0) {
