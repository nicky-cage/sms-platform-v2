...
// Get 获得Redis
func Get(platform string, args ...*redis.Conn) *redis.Conn {
	if len(args) > 0 {
		return args[0]
	}
	return GetConn(platform)
}

// Close 放回redis
func Close(platform string, conn *redis.Conn, args ...*redis.Conn) {
	if len(args) > 0 {
		return
	}
	ReturnConn(platform, conn)
}

// Restore 放回redis
func Restore(platform string, conn *redis.Conn, args ...*redis.Conn) {
	if len(args) > 0 { // 表示是长连接, 暂时不需要关闭, 调用时一定要注意
		return
	}
	ReturnConn(platform, conn)
}
...
// AllRedis 所有redis
func AllRedis() map[string]*redis.Conn {
...
// AllRedisRestore 关闭所有redis
func AllRedisRestore(all map[string]*redis.Conn) {
