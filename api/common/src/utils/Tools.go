...
// StructToMap 结构转map,因为struct不能循环,map可以.
...
// StructToSlice 结构转map,因为struct不能循环,map可以.
func StructToSlice(request interface{}, tagInfo string) []string {
	var result []string
...
// SendSMS 发送验证码短信
func SendSMS(phone string, code string, tmp int) error {
...
func GetBillNo(prefix string, args ...int) string {
	n := 0
	if len(args) == 0 {
		n = 5
	}
...
// GetBillStr 生成订单号数字
func GetBillStr(prefix string, args ...int) string {
	n := 0
	if len(args) == 0 {
		n = 5
	}
