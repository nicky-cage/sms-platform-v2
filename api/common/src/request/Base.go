		...
		case string:
			...
			case "between|timestamp":
				setQueryCondBetween(c, k, func(val interface{}) {
					...
					cond = cond.Filter(elastic.NewRangeQuery(k).Gte(utils.GetTimestampByString(val.(string))))
				}, func(val interface{}) {
					cond = cond.Filter(elastic.NewRangeQuery(k).Lte(utils.GetTimestampByString(val.(string))))
				})
			case "[]|timestamp":
				setQueryCondBetween(c, k, func(val interface{}) {
					cond = cond.Filter(elastic.NewRangeQuery(k).Gte(utils.GetTimestampByString(val.(string))))
	...
	for k, s := range data {
		...
		case string:
			...
			case "between|timestamp": // 2个字段
				setQueryCondBetween(c, k, func(val interface{}) {
					cond = cond.And(builder.Gte{k: utils.GetTimestampByString(val.(string))})
				}, func(val interface{}) {
					cond = cond.And(builder.Lte{k: utils.GetTimestampByString(val.(string))})
				})
			case "[]|timestamp": // 2个字段
				setQueryCondBetween(c, k, func(val interface{}) {
					cond = cond.And(builder.Gte{k: utils.GetTimestampByString(val.(string))})
			...
			case "[_]|timestamp": // 1个字段拆分 => 比如 created, 传过来的是时间区间 2020-01-02 - 2020-02-02 => 转换为时间戳对比
				...
				if exists {
					...
					startAt = utils.GetTimestampByString(areas[0])
					endAt = utils.GetTimestampByString(areas[1])
