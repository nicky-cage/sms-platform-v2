...
func sendSMPP(channel *CUGSender) {
	var wg sync.WaitGroup

	wg.Add(1)
	go sendingAndReceiveSMS(&wg, channel)

	wg.Wait()

}

func sendingAndReceiveSMS(wg *sync.WaitGroup, channel *CUGSender) {
	defer wg.Done()

	count := len(channel.IPList)         // 最大数量
	channelID := utils.RandInt(0, count) // 渠道编号
	remoteHost := fmt.Sprintf("%s:%d", channel.IPList[channelID], channel.Port)
	auth := gosmpp.Auth{
		SMSC:       remoteHost,
		SystemID:   channel.Account,
